...
Function CreateGroups()
{
  ...
  Try
  {
    ...
    Foreach ($ClassRecord in $Datatable)
    {
      ...
      # [ Folder creation ]
      $GroupFolder = $ClassRecord.klas 
      # Test if the remote folder exists
      $FolderExists = Test-Path -Path "$FileserverLLN\$MainFolder\$GroupFolder"
      If (-not $FolderExists)
      {
        # If the class folder doesn't exist we create it
        New-Item -Path "$FileserverLLN\$MainFolder\" -Name $GroupFolder -ItemType 'Directory'
        Write-Host "creating class"
      }

      # Test if the remote folder exists
      $FolderExists = Test-Path -Path "$FileserverLLN\$MainFolder\$GroupFolder\Opdrachten"
      If (-not $FolderExists)
      {
        # If the opdrachten folder doesn't exist we create it
        New-Item -Path "$FileserverLLN\$MainFolder\$GroupFolder" -Name 'Opdrachten' -ItemType 'Directory'
      ...
      $ACLOpdrachten = Get-Acl -Path "$FileserverLLN\$MainFolder\$GroupFolder\Opdrachten"
     ...
     $ACLArguments = $ACLIdentity, $ACLRights,$ACLInheritanceFlags, $ACLPropagationFlags,$ACLType
      ...
      Set-Acl -Path "$FileserverLLN\$MainFolder\$GroupFolder\Opdrachten" -AclObject $ACLOpdrachten
      #-----------------------------------------------------------------------------------------
      ...
      Get-Item "$FileserverLLN\$MainFolder\$GroupFolder\Opdrachten"
    ...
    Foreach ($Record in $Datatable)
     {
       ...
       If ($CurrentName -eq $PreviousName)
       {
        ...
        $HomeDirectory="$FileserverLLN\$MainFolder\$GroupFolder\$CurrentName"
        $Password = ConvertTo-SecureString "Abcde123" -AsPlainText -Force

        

        $FolderExists = Test-Path -Path "$FileserverLLN\$MainFolder\$GroupFolder\$CurrentName"
        If (-not $FolderExists)
        {
          # If the class folder doesn't exist we create it
          New-Item -Path "$FileserverLLN\$MainFolder\$GroupFolder\" -Name $CurrentName -ItemType 'directory'
       ...
       {
        New-ADUser -Name $Email -SamAccountName $CurrentName -GivenName $GivenName -Surname $Surname -UserPrincipalName $Email -DisplayName $Displayname -EmailAddress $Email -Path "OU=$GroupName,OU=leerlingen,$DC" -Description $Description -ScriptPath $LogonScript -Title $Description -Company 'GO! Handelsschool Aalst' -HomeDirectory $HomeDirectory -HomeDrive 'U:' -Enabled $True -AccountPassword $Password -ChangePasswordAtLogon:$true
